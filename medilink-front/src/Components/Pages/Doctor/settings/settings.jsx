...
export default function DoctorSettings() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const requests = [
          api.get("/doctor/settings/profile").catch(err => ({ error: err, type: 'profile' })),
          api.get("/doctor/settings/security").catch(err => ({ error: err, type: 'security' })),
          api.get("/doctor/settings/preferences").catch(err => ({ error: err, type: 'preferences' })),
        ...
        if (profileRes.error || securityRes.error || preferencesRes.error || userRes.error) {
          const errorList = [];
          if (profileRes.error) errorList.push(`Profile: ${profileRes.error.message}`);
          if (securityRes.error) errorList.push(`Security: ${securityRes.error.message}`);
          if (preferencesRes.error) errorList.push(`Preferences: ${preferencesRes.error.message}`);
          if (userRes.error) errorList.push(`User: ${userRes.error.message}`);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      const response = await api.put("/doctor/settings/profile", profileForm);
  ...
  const handleTogglePreference = async (key, value) => {
    try {
      ...
      await api.put("/doctor/settings/preferences", { [key]: value });
  ...
  const handleDeactivateAccount = async () => {
    ...
    try {
      await api.post("/doctor/settings/deactivate");
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", height: "100vh", background: darkMode ? "#0f172a" : "#f6f6f8", overflow: "hidden" }}>
        <Sidebar dark={darkMode} setDark={setDarkMode} />

        <main style={{ flex: 1, marginLeft: 260, padding: "2rem 3rem", maxWidth: 900, overflowY: "auto", height: "100vh" }}>
          ...
          {/* Profile Section */}
          <Section darkMode={darkMode} title="Profile Settings" subtitle="Update your public information and professional details.">
            {loading ? (
              ...
              <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "0.5rem", padding: "1rem", color: "#dc2626", marginBottom: "1rem" }}>
                ...
                <button 
                  ...
                  style={{ padding: "0.5rem 1rem", background: "#dc2626", color: "white", border: "none", borderRadius: "0.375rem", cursor: "pointer", fontSize: "0.875rem", fontWeight: 500 }}
                >
                  Retry
                </button>
              </div>
              ...
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                  {[
                    ...
                    { label: "Medical License", key: "license_number", type: "text", placeholder: "ML-XXXX-XXX" },
                    { label: "Hospital/Clinic", key: "hospital_name", type: "text", placeholder: "Hospital name" },
                    { label: "Years of Experience", key: "years_of_experience", type: "number", placeholder: "10" },
                  ].map(field => (
                  ...
                  <div style={{ gridColumn: "1 / -1" }}>
                    <label style={{ fontSize: "0.8rem", fontWeight: 600, color: darkMode ? "#94a3b8" : "#475569" }}>About / Bio</label>
                    <textarea
          ...
          <Section darkMode={darkMode} title="Platform Preferences" subtitle="Customize your notification and application experience.">
            {loading ? (
              ...
              <>
                <div>
                  <div style={{ fontWeight: 700, fontSize: "0.8rem", color: darkMode ? "#94a3b8" : "#475569", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "1rem" }}>
